...
/**
 * Definition class for table lang.
 *
 ...
 * @version $Id$
 *
 * @method Application_Model_Lang_Row createRow(array $data, string $defaultSource = null)
 * @method Application_Model_Lang_Rowset fetchAll(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $count = null, int $offset = null)
 * @method Application_Model_Lang_Row fetchRow(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $offset = null)
 * @method Application_Model_Lang_Rowset find()
 *
 */
abstract class Application_Model_Lang_DbTable_Abstract extends Zend_Db_Table_Abstract
{
    /**
     * @var string
     */
  ...
  0 => 'Application_Model_EtablissementLang_DbTable',
  1 => 'Application_Model_UniversiteLang_DbTable',
     ...
     * @var string
     */
    protected $_rowClass = 'Application_Model_Lang_Row';

    /**
     * @var string
     */
    protected $_rowsetClass = 'Application_Model_Lang_Rowset';
  ...
  'deleted' => '0',
    ...
    public function getDbSelectByParams($params = array(), $sortField = '', $sortOrder = '')
    {
        ...
        if (isset($params['idlang']) && !empty($params['idlang'])) {
            $select->where('idlang = ?', $params['idlang']);
        }

        if (isset($params['iso_code']) && !empty($params['iso_code'])) {
            $select->where('iso_code = ?', $params['iso_code']);
        }
        ...
        if (isset($params['_kw']) && !empty($params['_kw'])) {
            ...
            if ('all' === $searchMode || 'iso_code' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('iso_code LIKE ?', "%$keywords%");
            }
