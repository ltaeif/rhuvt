 ...
 *
 */
class Default_MutationController extends Zend_Controller_Action
{	
	...
	public function verifAction()
	{
		...
		$tableDemande = new Application_Model_Mutation_DbTable();
		
		 $row = $tableDemande->find($id)->current();

        if (!$row) {
                $this->_helper->redirector("create","Mutation","default",array('demande_codedem'=>$id,'type'=>$type));
            exit;
        }
		
		 $this->_helper->redirector("updatedemande","Mutation","default",array('demande_codedem'=>$id,'type'=>$type));
	}
